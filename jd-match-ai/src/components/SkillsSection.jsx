/**
 * SkillsSection.jsx
 * Reusable component that renders matched and missing skills as
 * colour-coded badge chips.
 */
...
function SkillBadge({ skill, variant }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors cursor-default ${styles[variant]}`}
    >
      {variant === 'matched'
...
/* ── Card wrapper ───────────────────────────────────────────────────────── */
function SkillCard({ title, icon, skills, variant, emptyMessage }) {
  return (
